...
from MuseModels.ImplicitCCF import ImplicitCCF
...
if __name__ == '__main__':
    ...
    myRank = [15]
    myLambda = [1.0]
    myAlpha = [0.0000005] # 0.0001 sweet spot                                                                                                                                                                  
    myIteration = [50]
    homeDir = expanduser("~")
    ...
    logFileName = logFileFolder + '/CCF-muse-MSD-step%s' % myStep + configModel + '.log'
    # you can input some starter model for the CF                                                                                                                                                            
    starterModelUmf = logFileFolder + '/CCF-muse-MSD-step%s' % myPreviousStep + configModel + '.log_Umf.npy'
    starterModelInf = logFileFolder + '/CCF-muse-MSD-step%s' % myPreviousStep + configModel + '.log_Inf.npy'
    ...
    conf = SparkConf().set("spark.deploy.defaultCores", 2).set("spark.cores.max", 10000000).set("spark.executor.memory", "8G")\
        .setMaster("spark://10.118.132.141:7077").setAppName("CCF-muse-HDFS-MSD")
    sc = SparkContext(conf=conf)
    # binary rating, explicit rating
    #ratingRDD = sc.textFile("hdfs://ec2-52-41-137-208.us-west-2.compute.amazonaws.com:9000/MSD_data/train_data_sorted.csv").map(lambda r: r.split()).map(lambda r: ( (int(r[0]), int(r[1])) , 1.0  )  ).repartition(100)

    # implicit rating - play counts
    # train_data_sorted.csv
    # train_data_sorted.csv_train
    # train_data_sorted.csv_test

    ratingRDD = sc.textFile("hdfs://ec2-52-41-137-208.us-west-2.compute.amazonaws.com:9000/MSD_data/train_data_sorted.csv").map(lambda r: r.split()).map(lambda r: ( (int(r[0]), int(r[1])) , float(r[2])  )  ).repartition(100)
    ...
    if myStep > 0:
        ...
        myCCF = ImplicitCCF(logFileName = logFileName, starterModel = [ starterModelUmf, starterModelInf ], sc = sc )
    else:
        #myCCF = CCF(logFileName = logFileName, starterModel = None , sc = sc )
        myCCF = ImplicitCCF(logFileName = logFileName, starterModel = None , sc = sc )
