...
if __name__ == '__main__':
    ...
    logFileName = logFileFolder + '/CCF-muse-1M-step%s' % myStep + configModel + '.log'
    ...
    starterModelUmf = logFileFolder + '/CCF-muse-1M-step%s' % myPreviousStep + configModel + '.log_Umf.npy' 
    starterModelInf = logFileFolder + '/CCF-muse-1M-step%s' % myPreviousStep + configModel + '.log_Inf.npy' 
    trainOrPredict = 'train'
    localOrRemoteCluster = 'remote'
    if trainOrPredict == 'predict':
        localOrRemoteCluster = 'local'
        numPartitions = 4
        starterModelUmf = logFileFolder + '/CCF-muse-1M-step%s' % myStep + configModel + '.log_Umf.npy'
        starterModelInf = logFileFolder + '/CCF-muse-1M-step%s' % myStep + configModel + '.log_Inf.npy'
    ...
    ratingRDD = df.select('user_id', 'song_id', 'rating').map(convertRatingToDecimal)
    ratingRDD.cache()
