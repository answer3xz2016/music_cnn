...
class CCF(object):
    ...
    def train(self, ratings, rank, iterations=5, lambda_=0.01, alpha=0.01, blocks=1, nonnegative=False):
        ...
        def cofiCostFunc(r, UmfBr, InfBr, lambdaBr, alphaBr):
            ...
            InfPart_grad =  Delta.transpose() * UmfPart  #+ lambda_ * InfPart
            #print '\n\nDebug: InfPart_grad, ', InfPart_grad, InfPart_grad.shape

            # (R - Y) * I + lambda * U
            # Theta_grad = (Theta*X' - Y').*R' * X + lambda*Theta;
            UmfPart_grad =  Delta * InfPart #+ lambda_ * UmfPart
        ...
        for i in range(0,iterations):
            ...
            weights = ratings.mapPartitions(partial(cofiCostFunc,UmfBr=UmfBr,InfBr=InfBr,lambdaBr=lambdaBr,alphaBr=alphaBr )).reduceByKey(add).collect()
